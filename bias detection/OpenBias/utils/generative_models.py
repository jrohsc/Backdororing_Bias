    ...
    def __init__(
        ...
        if gen_info['version'] == 'stabilityai/stable-diffusion-2':
            ...
            self.dm = StableDiffusionPipeline.from_pretrained(
                #gen_info['version'], 
'/work/pi_ahoumansadr_umass_edu/sd_finetune/diffusers/examples/text_to_image/epoch_config_doctor_50',
                scheduler=scheduler,
                torch_dtype=torch.float16,
                use_safetensors=True
            ).to(device)
            ...
            # diffusion model
            self.dm = StableDiffusionPipeline.from_pretrained('/work/pi_ahoumansadr_umass_edu/sd_finetune/diffusers/examples/text_to_image/epoch_config_doctor_50', 
                torch_dtype=torch.float16,
                use_safetensors=True
            ).to(device)
