...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        vae = AutoencoderKL.from_pretrained(
            args.pretrained_model_name_or_path, cache_dir=args.cache_dir, subfolder="vae", revision=args.revision
        )

    unet = UNet2DConditionModel.from_pretrained(
        args.pretrained_model_name_or_path, cache_dir=args.cache_dir, subfolder="unet", revision=args.non_ema_revision
    )
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            ...
            for i in range(len(models)):
                ...
                load_model = UNet2DConditionModel.from_pretrained(input_dir, cache_dir=args.cache_dir, subfolder="unet")
    ...
    if args.dataset_name is not None:
        ...
        ####### Dataset Tune #######
        # dataset_train = load_from_disk(args.poison_dataset_path)    
        dataset_train = load_from_disk(args.poison_dataset_path)
        # args.output_dir = args.output_dir + str(args.learning_rate)
        print("base model: ", args.pretrained_model_name_or_path)
